    ...
    methods: {
        ...
        setEventFields(){
            this.current_events.forEach((event, index) => {
                ...
                $(`#target_column__${index}`).val(event.values.new_value)
                $(`#source_column__${index}`).val(event.values.old_value)
            });
        },
        ...
        saveEventHandler(event){
            ...
            if (isNew==false){
                ...
                axios.post(proxyCallUrl, callMeta)
                    .then(proxyResp => {
                        resp = proxyResp.data['response']
                        statusCode = proxyResp.data['response_code']
                        if (statusCode != 200){
                            showNotify("ERROR", data['error'])
                            return
                        }
